...
local function CreateGuildFrame()
    ...
    guildFrame:SetScript("OnShow", function()
        ...
        guildFrame:SetBackdropColor(0.05, 0.05, 0.05, 0.95)
        guildFrame:SetBackdropBorderColor(0, 0, 0, 1)
        ...
        local motdSize = db.fontSize + 1
        local fontFlags = "OUTLINE"
        local r, g, b = GetColor()
        
        -- Update title
        guildTitle:SetFont(fontPath, titleSize, fontFlags)
        guildTitle:SetTextColor(r, g, b)
        
        -- Update MotD with larger font
        guildMotD:SetFont(fontPath, motdSize, fontFlags)
        
        -- Update footer
        guildFooter:SetFont(fontPath, bodySize, fontFlags)
        
        -- Update headers
        for _, fs in ipairs(guildHeaderRefs) do
            fs:SetFont(fontPath, bodySize, fontFlags)
